...
import { downloadAndBundle } from "../downloader";
...
async function runJob(id: string, inputPath: string): Promise<void> {
  logger.log(`[job:${id}] starting download…`);
  const payload = JSON.parse(readFileSync(inputPath, "utf-8")) as PythonPayload;
  await downloadAndBundle(id, payload);
